...
const cards = [
  ["学习板块", "categories", FolderOpen, "from-violet-500 to-purple-400"],
  ["学习文章", "articles", BookOpen, "from-cyan-500 to-blue-400"],
  ...
  return (
    <>
      ...
      <Card className="mt-7 overflow-hidden">
        <div className="border-b border-white/10 p-6"><h2 className="text-lg font-semibold">最近发布</h2></div>
        <div className="divide-y divide-white/5">
          {stats?.recentArticles.map((article) => (
            <div key={article.id} className="flex items-center justify-between gap-4 p-5">
              <div><p className="font-medium">{article.title}</p><p className="mt-1 text-xs text-slate-500">{article.category?.name}</p></div>
