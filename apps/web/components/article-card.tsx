...
export function ArticleCard({ article }: { article: Article }) {
  ...
  return (
    <Link href={`/articles/${article.slug}`} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200/70">
      ...
      <div className="p-5">
        <h3 className="line-clamp-1 text-lg font-bold text-slate-900 transition group-hover:text-blue-600">{article.title}</h3>
        <p className="mt-2 line-clamp-2 min-h-11 text-sm leading-6 text-slate-500">{article.summary ?? "打开文章继续阅读。"}</p>
